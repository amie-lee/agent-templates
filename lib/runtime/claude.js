...
function validateClaudeBinary(binary) {
  let resolved = binary;
  try {
    resolved = fs.realpathSync(binary);
  } catch (error) {
    throw new Error(`Claude Code binary path is invalid: ${binary}`);
  }

  const contents = readTextIfPresent(resolved);
  if (contents.includes("claude native binary not installed")) {
    throw new Error(
      [
        "Claude Code CLI is installed, but its native binary is missing.",
        "Reinstall @anthropic-ai/claude-code without --ignore-scripts/--omit=optional, or reinstall the Claude Code CLI.",
      ].join(" ")
    );
  }
  ...
  try {
    ...
    throw new Error(`Claude Code binary is not executable: ${resolved}`);
  }
...
function runAgent(startDir, agentName, options = {}) {
  ...
  if (launch.error) {
    throw new Error(`Failed to launch Claude Code: ${launch.error.message}`);
  }

  if (launch.signal) {
    throw new Error(`Claude Code exited due to signal ${launch.signal}.`);
  }
